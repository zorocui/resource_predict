...
- Treat production deployment scale as the design baseline: assume thousands to tens of thousands of resources in an internal network, with K8S container-level metrics multiplying the forecasting workload. Do not use the 45-resource mock dataset as the basis for architecture, performance, storage, or model-lifecycle decisions.
