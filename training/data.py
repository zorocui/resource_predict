...
def load_series(raw: Path, resource_type: str, metric: str, level: str,
    ...
    for rid in ids:
        ...
        for container, metrics in groups:
            ...
            if (len(series) < 3 or series.index.hasnans or not series.index.is_unique
                ...
                skipped.append({"resource_id": rid, "container": container, "reason": "irregular_or_invalid"})
                continue
            selected.append(SeriesData(rid, container, series))
        if not found:
    ...
    for item in selected:
        digest.update(repr((item.resource_id, item.container)).encode("utf-8"))
